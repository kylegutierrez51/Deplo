...
Between `markStageRunning` and `finishStage` the row's `logSnippet` is null, so a stage that has been building for four minutes shows the Run Detail page nothing. `execute` therefore takes an `onSnapshot` callback and offers it the tail every `snapshotMs` (2s), which `stageProcessor` persists through `db.recordStageProgress`. Three things make that safe, and none is optional. The snapshot reads the buffer through `peek`, **not** `flush` — `flush` banks the partial-line remainder and clears it, so a tick landing mid-line would split one line of output across two in the *final* snippet, an observer corrupting what it was only meant to read. `recordStageProgress` guards on `status: 'RUNNING'` like every other write in `db.ts`, which is what stops a snapshot still in flight when the command exits from overwriting the terminal row `finishStage` just wrote, timeout note and all. And the callback is fire-and-forget: nothing awaits it, it drops a tick while the previous write is still in flight (two overlapping updates can commit out of order and leave the older tail as the row's last word), and it swallows its own rejection, because it runs from a timer where an unhandled rejection has no owner and would take the runner down over a database hiccup. A tick whose tail has not changed writes nothing at all.
...
Every phase is individually wrapped, since all of this happens before `worker.run()` and an unhandled rejection would stop the runner from booting rather than merely leaving work unrecovered.

`runner/sweeper.ts` covers what boot-only recovery cannot. A run job that exhausts its attempts leaves no trace on the run row — `attempts: 3` with a 2s exponential backoff means six seconds of Postgres being unreachable spends the whole budget — and the job lands in the failed set with the run stuck at `QUEUED` while the runner processes everything else. Nothing in the app rescues it either: `RETRYABLE` refuses to re-run anything unfinished, so the only thing anyone can do with it is cancel it. `startStalledRunSweep` re-enters unfinished runs on a timer, and the same pass closes the window where an approval decision commits but its enqueue fails.

**It is deliberately not the reaper on a timer.** `reapAbandonedWork` is only safe before `worker.run()`, and the parts that make it unsafe are exactly the ones that would help least here: `reclaimStageJob` deletes a job's BullMQ lock, and `reapStaleStages` fails every `RUNNING` row it finds. The repeatable subset is the last phase alone — read the unfinished runs and hand them back to the scheduler — because every write reachable from there is a compare-and-swap and `readyStages` is idempotent, so re-entering a healthy run reads the row and does nothing. The cutoff in `findStalledRuns` is a cost filter, not a safety one; `QUEUED` is measured from `createdAt` and `RUNNING` from `startedAt` because a queued run has no `startedAt` and a single-column filter would silently never match one.

**Every await on the queue from the web app is bounded** (`lib/queue/timeout.ts`). A Redis command is not a request that fails when Redis is down: `queueConnection()` sets `maxRetriesPerRequest: null` explicitly — BullMQ does not default to that for a plain `Queue`, only for the dedicated blocking connection a `Worker` opens for itself — and with it set, ioredis buffers the command and retries the connection forever, so `queue.add()` returns a promise that never settles in either direction. Unbounded, the server action awaiting it hangs and the page renders forever with no error — every `catch` on the trigger path is unreachable. `enqueuePipelineRun` therefore races a 5s timer and `getRunnerAvailability` a 500ms one. `enableOfflineQueue: false` is the tempting alternative and the wrong one: the queue is constructed lazily, so the first enqueue after a server start would be issued mid-handshake and reject spuriously.

A trigger whose enqueue fails is handled at the source instead: `lib/actions/run-trigger.ts` deletes the row it just wrote, since a run that was never enqueued has no stage rows and `addNodeDetails` renders a node with no row as `pending` — writing it `FAILED` would show a failed run whose every stage reads "Pending". An *approval* whose enqueue fails is not undone, because that write is a decision someone made; the sweep is what recovers it.

**`runner/connection.ts` sets the same `maxRetriesPerRequest: null`, for a sharper reason than the web app's.** `advanceRun`'s dispatch is two writes, not one transaction: `claimStageForQueue` claims a stage in Postgres (`PENDING` → `QUEUED`) and only then does `enqueueStageJob` try Redis. Left at ioredis's real default of 20, a sustained outage — a few minutes, per ioredis's own backoff curve — exhausts that budget and `enqueueStageJob` rejects with `MaxRetriesPerRequestError`. Nothing rolls the Postgres write back when that happens, and `readyStages` only ever reconsiders a `PENDING` row, so the stage is left `QUEUED` with no job behind it and no path back — invisible to every future call, including the sweeper's own. Setting the connection to buffer instead of give up turns that outage back into a delay a reconnect resolves on its own, the same way a delayed approval enqueue resolves itself once Redis returns.
