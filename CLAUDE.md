...
Failures split into retryable and not: a command that ran and failed (or timed out) may go differently next time, one that never started — an unresolvable secret, a child that would not spawn — will not, and burning the budget on it only buries the message. `runner/reaper.ts` runs once at boot, before `worker.run()`: it fails rows left `RUNNING` by a dead process, then re-enters every unfinished run (`advanceRun` for `RUNNING`, `processRun` for `QUEUED`, which may never have been materialized). It **assumes a single runner process**, and it is what replaces the automatic redelivery `maxStalledCount: 0` turns off. Two known gaps: it runs only at boot, so a run stalled by a transient Postgres or Redis error stays stalled until someone restarts the runner; and a `QUEUED` *stage* row whose job was already active at death is not recovered, because distinguishing that from one still waiting in Redis needs a queue round trip per row.
