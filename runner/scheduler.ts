...
/** Nothing has claimed this stage: no row yet, or a row still at PENDING. Narrower than 'in-flight'. */
...
export function readyStages(graph: GraphJson, outcomes: Outcomes): string[] {
  ...
  // succeeded vetoes its children. One pass over the forward index, no reverse map.
  ...
  // Candidates come from graph.nodes, not from the maps: buildMaps seeds an entry for
  // every edge target, including a dangling one naming an id that is not a node.
  return graph.nodes
    .map(n => n.id)
